...
for i in range(len(prog_files)) :
        ...
        to_append = f'{filename} {genre} {np.mean(chroma_stft)} {np.mean(rmse)} {np.mean(spec_cent)} {np.mean(spec_bw)} {np.mean(rolloff)} {np.mean(zcr)}'    

        # Append all mfcc features i.e., 20 rows
        for e in mfcc:
            to_append += f' {np.mean(e)}'
        
        file = open('data_features.csv', 'a', newline='')
        with file:
            writer = csv.writer(file)
